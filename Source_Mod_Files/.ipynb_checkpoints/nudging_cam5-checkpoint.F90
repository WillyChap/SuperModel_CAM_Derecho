...
!=====================================================================
  ...
  public:: First_Nudge !++WEC

  ! Nudging Parameters
  !--------------------
  logical          :: Nudge_Model       =.false.
  logical          :: Nudge_ON          =.false.
  logical          :: First_Nudge       =.true. !++WEC
...
contains
  ...
  subroutine nudging_readnl(nlfile)
   ...
   Nudge_ON          =.false.
   First_Nudge       =.true. !+++WEC
   Nudge_Beg_Sec=0
   Nudge_End_Sec=0
   Nudge_First_Sec=100 !+++WEC
   ...
   call mpibcast(First_Nudge        , 1, mpilog, 0, mpicom) !++WEC
  ...
  subroutine nudging_init
   ...
   if(masterproc) then
     ...
     write(iulog,*) 'NUDGING: First_Nudge=',First_Nudge !++WEC
   ...
   if((Before_End).and.((Update_Nudge).or.(Update_Model))) then
     ...
     elseif(Nudge_Force_Opt.eq.1) then
       ...
       do lchnk=begchunk,endchunk
         ...
         Target_U(:ncol,:pver,lchnk)=(1._r8)*Nobs_U(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
                                        +0._r8 *Nobs_U(:ncol,:pver,lchnk,Nudge_ObsInd(2))
         Target_V(:ncol,:pver,lchnk)=(1._r8)*Nobs_V(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
                                        +0._r8 *Nobs_V(:ncol,:pver,lchnk,Nudge_ObsInd(2))
         Target_T(:ncol,:pver,lchnk)=(1._r8)*Nobs_T(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
                                        +0._r8 *Nobs_T(:ncol,:pver,lchnk,Nudge_ObsInd(2))
         Target_Q(:ncol,:pver,lchnk)=(1._r8)*Nobs_Q(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
                                        +0._r8 *Nobs_Q(:ncol,:pver,lchnk,Nudge_ObsInd(2))
         Target_PS(:ncol     ,lchnk)=(1._r8)*Nobs_PS(:ncol     ,lchnk,Nudge_ObsInd(1)) &
                                        +0._r8 *Nobs_PS(:ncol     ,lchnk,Nudge_ObsInd(2))
         !---WEC                                  
...
!    endif
  ...
  subroutine nudging_update_analyses_fv(anal_file)
   ...
   if(masterproc) then
     ...
     write(*,*) 'PAUSERESUME: Call ./Fake_DA_CAM5.py'
     call system('./Fake_DA_CAM5.py')
     do while ( lpause )
  ...
  if(After_First) First_Nudge = .false.
   
  end subroutine  ! zero_nudging
  
  subroutine flip_first_nudge(state)
   use physics_types,only: physics_state
   use constituents ,only: cnst_get_ind
   use ppgrid       ,only: pver,pcols
  ...
  end subroutine  ! flip_first_nudge
